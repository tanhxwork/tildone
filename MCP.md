...
| **Evidence & links** | A task's branch / PR / commit / worktree URLs **and** local file evidence (a screenshot, a spec, a PDF), shown as clickable chips in the task's **Evidence & links**. Add with `add_link`, remove with `delete_link`; returned by `get_task` as `links`. **You** supply the `url` (or file path) and `label` — only the caller standing in the checkout knows the remote, the repo, and where the file lives. `http(s)` URLs open in the browser; an absolute path to an evidence file (`md`/`txt`/`html`/`png`/`jpg`/`gif`/`svg`/`webp`/`pdf`/`json`/`csv`/`log`) opens in its default app. URL chips show on the card; file chips live in the task detail only. |
...
| `add_link` | `task_id`, `url` (an `http(s)` URL **or** an absolute path to an evidence file); optional `label`, `kind` (`pr`/`branch`/`commit`/`worktree`/`other` — ignored for a file, which is always kind `file`) | Attaches a link or a file, clickable in the task's Evidence & links. A URL opens in the browser; a file (extensions `md`/`txt`/`html`/`htm`/`png`/`jpg`/`jpeg`/`gif`/`svg`/`webp`/`pdf`/`json`/`csv`/`log`, `~` expanded) opens in its default app. `label` defaults to the URL's last path segment or the file's basename. Non-http URL schemes, relative paths, and any other extension are refused. |
